...
**Figure:** Example of the UI that the script generate.
...
As shown in the resolution guide, the higher the number of pixels, the longer the compilation time becomes. Not only that, but increasing the image resolution also increases the amount of code automatically generated by LaTeX

![Sol](images/Sol.jpg)
**Figure:** Input image as a example of the use of the aplication.

As we saw in the user interface screenshot, we can select the file from any desired location, and the code will be automatically generated in a text document. As shown in the following images, there is a slider that allows us to select the desired quality or pixel density for the conversion. To illustrate this process, we will display the results of the slider at its maximum and minimum settings.

![Max_gen](images/Max_gen.png)

**Figure:** Example of the UI with the Slider in the maximun resolution.

![Lower_gen](images/Lower_gen.png)

**Figure:** Example of the UI with the Slider in the minimun resolution.

As you can see in the new pop-up image, in both cases, setting the slider to maximum generates around 12,600 colored rectangles, whereas at the minimum setting, around 144 colored rectangles are generated. Therefore, it is recommended to select a midpoint on the slider, such as 64px, since the compilation time is actually more significant when compiling the code in LaTeX rather than in Python.

This difference in resolution can be seen below, illustrating the difference between the maximum and minimum points.

![Im_max](images/Im_max.png)

**Figure:** Example of the input image at his maximun resolution.

![Im_min](images/Im_min.png)

**Figure:** Example of the input image at his minimun resolution.

***

## Conclusion
As you can see, the image generated by the LaTeX code is of lower quality than the input image. However, this code logic could be expanded to achieve the same quality and resolution as the input image, thereby eliminating the need for it.

Although the previous statement is technically correct, continuing or expanding this work would be pointless—at least under the current logic—because code generation in LaTeX is extremely extensive, which would only complicate the compilation of a large LaTeX document and result in an absurd amount of code. 

In the following images, we can see the difference in code generation in LaTeX: while it generates at least 150 lines of code at its minimum, it generates more than 12,000 lines at its maximum, which corresponds to the number of rectangles drawn in the Python code logic. 

![Min_latex_lines](images/Min_latex_lines.png)
**Figure:** Example of Line number of the output image at his minimun resolution.

![Max_latex_lines](images/Max_latex_lines.png)
**Figure:** Example of Line number of the output image at his maximun resolution.

And although this could be solved by creating a different number of files in LaTeX, this would simply replace the option of adding the original input images with the same number of code files. Therefore, it does not achieve the goal of replacing images with code, but it can be extended as a way to preserve images in plain text, a way to modify specific pixels in the image, or as an exercise in manipulating pixels in input and output images, similar to a hardware line buffer.
***
